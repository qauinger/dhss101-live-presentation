<script lang="ts">
	...
	let hometownUsers = data.filter((d) => d.Q8 != '');

	hometownUsers.forEach((d) => {
		let c = getCoord(Number.parseInt(String(Number.parseInt(d.Q1 + ''))));
		if (c?.lat != undefined && c?.lon != undefined) {
			points.push({ pos: [c?.lon, c?.lat], color: likertToColor(Number.parseInt(d.Q8)) });
		}
	});
</script>

<Map title="Satisfaction of Hometown Transit Systems" data={points} />
<Chart
	data={[{ name: '1' }, { name: '2' }, { name: '3' }, { name: '4' }, { name: '5' }]}
	c="name"
	cScale={scaleOrdinal()}
	cRange={['red', 'orange', 'yellow', 'lightgreen', 'green']}
>
	<Legend
		title="Satisfaction"
		placement="bottom"
